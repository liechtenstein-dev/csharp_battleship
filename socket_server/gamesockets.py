    ...
    try:
      while True:
        ...
        if(b.state_game == True):
          self.state = True
          connection.sendall(("state changed to 'True' destroying all sockets").encode("utf-8"))
          self.remove(connection)
          self.close_all()
          print("Sockets remaining into server socket: ", len(self.clients_connection_list))
          break
