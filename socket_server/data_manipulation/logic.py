...
class AttackAlgorithm:
    ...
    def hunting_attacks(self):
        ...
        if self.hits:
            ...
            self.attack_row = random.randint(0, len(self.board) - 1)
            self.attack_col = random.randint(0, len(self.board[0]) - 1)
    ...
    def next_attack(self, hits):
        ...
        valid_attacks = [(row + dr, col + dc) for dr, dc in directions if 0 <= row + dr < len(self.board) and 0 <= col + dc < len(self.board[0])]
